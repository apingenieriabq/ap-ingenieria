    ...
    if(empty($validacion)){
      ...
      if(empty($Persona->personaID)){
        $Persona->personaID = $Persona->crear(
          ...
          $this->personaMUNICIPIO, $this->personaDIRECCION, $this->personaEMAIL,$this->personaTELEFONO, $this->personaCELULAR, $this->personaNIT, $URL_FOTO,
        ...
        if (!empty($Colaborador->colaboradorID)){
          ...
          if (!empty($Usuario->usuarioID)) {
            ...
            $Colaborador->Usuario = $Usuario;
              return Respuestassistema::exito('Nuevo Usuario/Colaborador creado Exitosamente. Los datos de inicio son: <br /> Usuario: <b>'.$Usuario->usuarioNOMBRE.'</b><br /> Clave: <b>'.$usuarioHASH.'</b>', $Colaborador);
          } else {
